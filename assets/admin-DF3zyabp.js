import{s as u}from"./supabase-DBx6agDF.js";async function ha(a,t){const{data:e,error:o}=await u.auth.signInWithPassword({email:a,password:t});if(o)throw new Error(o.message);const n=await V(e.user.id);return{user:e.user,profile:n}}async function B(){await u.auth.signOut()}async function ga(){const{data:a}=await u.auth.getSession();if(!a.session)return null;const t=await V(a.session.user.id);return{user:a.session.user,profile:t}}async function V(a){const{data:t,error:e}=await u.from("profiles").select("*").eq("id",a).single();if(e)throw new Error(`Perfil não encontrado: ${e.message}`);return t}async function $a({nome:a,email:t,password:e,role:o}){const{data:n}=await u.auth.getSession(),i=n==null?void 0:n.session,{data:l,error:p}=await u.auth.signUp({email:t,password:e,options:{data:{nome:a,role:o}}});if(p)throw new Error(`Erro ao criar usuário: ${p.message}`);if(!l.user)throw new Error('Usuário não foi criado. No painel do Supabase, vá em Authentication → Settings e desative "Confirm email".');i!=null&&i.access_token&&await u.auth.setSession({access_token:i.access_token,refresh_token:i.refresh_token});const{error:m}=await u.from("profiles").upsert({id:l.user.id,email:t,nome:a,role:o},{onConflict:"id"});if(m)throw new Error(`Usuário criado, mas erro no perfil: ${m.message}`);return l.user}async function W({classificacao:a="",status:t="",beneficio:e=""}={}){if(!u)throw new Error("Supabase não configurado.");let o=u.from("leads").select("*").order("created_at",{ascending:!1});a&&(o=o.eq("classificacao",a)),t&&(o=o.eq("status_atendimento",t)),e&&(o=o.eq("beneficio",e));const{data:n,error:i}=await o;if(i)throw new Error(`Erro ao buscar leads: ${i.message}`);return n||[]}async function G(a,t){if(!u)throw new Error("Supabase não configurado.");const{error:e}=await u.from("leads").update({status_atendimento:t}).eq("id",a);if(e)throw new Error(`Erro ao atualizar lead: ${e.message}`)}async function ya({lead_id:a,tipo:t="whatsapp",direcao:e="enviado",descricao:o,responsavel:n="",numero_lead:i=""}){if(!u)throw new Error("Supabase não configurado.");const{error:l}=await u.from("atendimentos").insert([{lead_id:a,tipo:t,direcao:e,descricao:o,responsavel:n,numero_lead:i}]);if(l)throw new Error(`Erro ao salvar atendimento: ${l.message}`)}async function Ea(a){if(!u)throw new Error("Supabase não configurado.");const{data:t,error:e}=await u.from("atendimentos").select("*").eq("lead_id",a).order("created_at",{ascending:!0});if(e)throw new Error(`Erro ao buscar atendimentos: ${e.message}`);return t||[]}function Sa(a,t){return u.channel(`atendimentos:lead:${a}`).on("postgres_changes",{event:"INSERT",schema:"public",table:"atendimentos",filter:`lead_id=eq.${a}`},o=>t(o.new)).subscribe()}async function wa(){const{data:a,error:t}=await u.from("profiles").select("*").order("created_at",{ascending:!1});if(t)throw new Error(`Erro ao listar usuários: ${t.message}`);return a||[]}async function Aa(a,t){const{error:e}=await u.from("profiles").update({ativo:t}).eq("id",a);if(e)throw new Error(`Erro ao atualizar usuário: ${e.message}`)}async function qa(a,t){const{error:e}=await u.from("profiles").update({role:t}).eq("id",a);if(e)throw new Error(`Erro ao atualizar perfil: ${e.message}`)}const Q="",La="",Ca="",K="";function M(){return!!Q}function Y(){return!!K}async function xa(a,t){const e=String(a).replace(/\D/g,""),o=e.startsWith("55")?e:`55${e}`,n=`${Q.replace(/\/$/,"")}/message/sendText/${Ca}`,i=await fetch(n,{method:"POST",headers:{"Content-Type":"application/json",apikey:La},body:JSON.stringify({number:o,textMessage:{text:t}})});if(!i.ok){const l=await i.json().catch(()=>({}));throw new Error(l.message||`Evolution API retornou status ${i.status}`)}return await i.json()}function Ua(a,t){const e=String(a).replace(/\D/g,"");window.open(`https://wa.me/${e}?text=${encodeURIComponent(t)}`,"_blank","noopener,noreferrer")}async function Na(a,t){return M()?(await xa(a,t),{canal:"evolution"}):(Ua(a,t),{canal:"wame"})}async function Ta(a){var x,z,H,D,R;if(!Y())throw new Error("Chave Groq não configurada.");const{etapa:t,nome:e,beneficio:o,situacao:n,classificacao:i,observacoes:l,documentos:p,mensagemAtual:m}=a,$=`Você é um atendente humanizado de um escritório de advocacia especializado em INSS.
...
Nunca use asteriscos para negrito — use o formato WhatsApp (*texto*).`,w={abertura:`Escreva uma mensagem de abertura calorosa para ${e}, que entrou em contato sobre *${o}* (situação: ${n}). Prioridade: ${i}. ${l?`Observação do lead: ${l}`:""}. Apresente o escritório e pergunte se pode atendê-la agora.`,qualificacao:`Escreva uma mensagem de qualificação para ${e} sobre *${o}*. Faça 2 perguntas objetivas para entender melhor a situação antes de pedir os documentos.`,documentos:`Escreva uma mensagem solicitando os seguintes documentos para ${e}: ${p==null?void 0:p.join(", ")}. Seja gentil e explique para que servem de forma resumida.`,encerramento:`Escreva uma mensagem de encerramento para ${e}, informando que os documentos foram recebidos e que o especialista entrará em contato em breve. Agradeça e seja encorajador.`,personalizada:`Melhore esta mensagem mantendo o tom humanizado, sem alterar informações essenciais:

${m}`},q=w[t]||w.personalizada,y=await fetch("https://api.groq.com/openai/v1/chat/completions",{method:"POST",headers:{"Content-Type":"application/json",Authorization:`Bearer ${K}`},body:JSON.stringify({model:"llama-3.3-70b-versatile",messages:[{role:"system",content:$},{role:"user",content:q}],temperature:.7,max_tokens:400})});if(!y.ok){const ba=await y.json().catch(()=>({}));throw new Error(((x=ba.error)==null?void 0:x.message)||`Groq retornou status ${y.status}`)}return((R=(D=(H=(z=(await y.json()).choices)==null?void 0:z[0])==null?void 0:H.message)==null?void 0:D.content)==null?void 0:R.trim())||""}function ka(a,{nome:t,beneficio:e,situacao:o,classificacao:n,documentos:i=[],observacoes:l=""}){const p=e.replace("-"," ");return{abertura:`Olá, ${t}! 👋
...
Conte conosco! ⚖️`}[a]||""}const A={aposentadoria:["RG e CPF (originais e cópias)","Carteira de Trabalho (CTPS) — todas as folhas utilizadas","Extrato do CNIS (Cadastro Nacional de Informações Sociais)","PIS/PASEP","Comprovante de residência atualizado","Certidão de nascimento ou casamento"],"auxilio-doenca":["RG e CPF","Laudo médico atualizado (com CID, assinado e carimbado)","Atestados e exames recentes","Extrato do CNIS","PIS/PASEP","Comprovante de residência"],"bpc-loas":["RG e CPF de todos os membros da família","Comprovante de residência atualizado","Declaração de composição e renda familiar","Laudo médico (para pessoa com deficiência — com CID)","Certidão de nascimento ou casamento","Número do NIS/CADUNICO (se possuir)"],"beneficio-negado":["RG e CPF","Carta de indeferimento do INSS","Todos os documentos do pedido original","Extrato do CNIS","Laudos ou documentos adicionais que embasem o recurso","Comprovante de residência"],outros:["RG e CPF","Extrato do CNIS","Comprovante de residência","Documentos específicos conforme orientação do especialista"]},Ia={aposentadoria:"Aposentadoria","auxilio-doenca":"Auxílio-doença","bpc-loas":"BPC/LOAS","beneficio-negado":"Benefício Negado",outros:"Outros"},f={leads:[],loading:!1,erro:"",filtroStatus:"novo",modal:null};let v=null,L=null;function Pa(a){return a?new Date(a).toLocaleString("pt-BR",{day:"2-digit",month:"2-digit",year:"numeric",hour:"2-digit",minute:"2-digit"}):"-"}function F(a){return String(a||"").replace(/^\+/,"")}function Ba(a){return`<span class="chip ${{Alta:"chip-alta",Média:"chip-media",Baixa:"chip-baixa"}[a]||"chip-baixa"}">${a}</span>`}function J(){return f.leads.filter(a=>!(f.filtroStatus&&a.status_atendimento!==f.filtroStatus))}function Ma(){return`
        ...
        </div>`}function X(a){return f.loading?'<div class="ad-feedback ad-loading">Carregando leads…</div>':f.erro?`<div class="ad-feedback ad-erro">${f.erro}</div>`:a.length?`
        ...
        </div>`:'<div class="ad-feedback ad-vazio">Nenhum lead encontrado.</div>'}function Oa(){const a=J();return`
        <div class="ad-layout">
            ${Ma()}
        ...
        <div class="docs-seletor">
            ...
            </div>`}).join("")}function oa(){const a=b[s.etapaAtual];if(!a)return"";const t=s.textos[a.id]??ta(a.id),e=s.mensagens.some(n=>n.etapa===a.id),o=s.etapaAtual===b.length-1;return`
        ...
        </div>`}function E(){const a=c==null?void 0:c.querySelector("#chat-editor");a&&(a.innerHTML=oa(),sa());const t=c==null?void 0:c.querySelector("#chat-hist");t&&(t.innerHTML=k(),t.scrollTop=t.scrollHeight);const e=c==null?void 0:c.querySelector(".etapas-lista");e&&(e.innerHTML=_())}function sa(){var t,e,o,n,i,l,p;const a=c==null?void 0:c.querySelector("#chat-editor");a&&((t=a.querySelector("#editor-msg"))==null||t.addEventListener("input",m=>{const $=b[s.etapaAtual];s.textos[$.id]=m.target.value}),(e=a.querySelector("#btn-ia"))==null||e.addEventListener("click",Ga),(o=a.querySelector("#btn-regenerar"))==null||o.addEventListener("click",()=>{const m=b[s.etapaAtual];delete s.textos[m.id],E()}),(n=a.querySelector("#btn-enviar"))==null||n.addEventListener("click",Qa),(i=a.querySelector("#btn-voltar"))==null||i.addEventListener("click",()=>{s.etapaAtual>0&&(s.etapaAtual--,E())}),(l=a.querySelector("#btn-proxima"))==null||l.addEventListener("click",()=>{s.etapaAtual<b.length-1&&(s.etapaAtual++,E())}),(p=a.querySelector("#btn-finalizar"))==null||p.addEventListener("click",Ka),a.querySelectorAll(".doc-check").forEach(m=>{m.addEventListener("change",()=>{var I;const $=((I=s.lead)==null?void 0:I.beneficio)||"outros",w=A[$]||A.outros;s.docsEscolhidos=[...a.querySelectorAll(".doc-check:checked")].map(x=>w[Number(x.dataset.i)]).filter(Boolean);const q=b[s.etapaAtual];s.textos[q.id]=ta(q.id);const y=a.querySelector("#editor-msg");y&&(y.value=s.textos[q.id])})}))}function Wa(){var a,t;(a=c==null?void 0:c.querySelector("#conversa-fechar"))==null||a.addEventListener("click",P),(t=c==null?void 0:c.querySelector("#conversa-modal"))==null||t.addEventListener("click",e=>{e.target.id==="conversa-modal"&&P()}),sa()}async function Ga(){var o;const a=b[s.etapaAtual],t=c==null?void 0:c.querySelector("#editor-msg"),e=(t==null?void 0:t.value)||"";s.melhorando=!0,E();try{const n=((o=s.lead)==null?void 0:o.beneficio)||"outros",i=s.docsEscolhidos,l=await Ta({etapa:a.id,nome:s.lead.nome,beneficio:ea(s.lead.beneficio),situacao:s.lead.situacao,classificacao:s.lead.classificacao,observacoes:s.lead.observacoes||"",documentos:i,mensagemAtual:e});s.textos[a.id]=l}catch(n){alert(`Erro na IA: ${n.message}`)}finally{s.melhorando=!1,E()}}async function Qa(){var o;const a=b[s.etapaAtual],t=c==null?void 0:c.querySelector("#editor-msg"),e=((t==null?void 0:t.value)||"").trim();if(!e){alert("Escreva a mensagem antes de enviar.");return}s.enviando=!0,E();try{const{canal:n}=await Na(s.lead.whatsapp,e);s.mensagens.push({etapa:a.id,texto:e,canal:n,sent:!0,ts:new Date().toISOString()});const i=String(s.lead.whatsapp||"").replace(/\D/g,"");await ya({lead_id:s.lead.id,tipo:"whatsapp",direcao:"enviado",descricao:`[${a.titulo}] ${e}`,responsavel:((o=s.profile)==null?void 0:o.nome)||"",numero_lead:i}),s.etapaAtual<b.length-1&&s.etapaAtual++}catch(n){alert(`Erro ao enviar: ${n.message}`)}finally{s.enviando=!1,E()}}async function Ka(){var a;try{await G(s.lead.id,"qualificado"),s.lead.status_atendimento="qualificado",(a=s.onLeadUpdated)==null||a.call(s,s.lead),P()}catch(t){alert(`Erro ao encerrar: ${t.message}`)}}async function Ya(){try{const e=await Ea(s.lead.id);s.mensagens=e.map(o=>{var n,i;return{etapa:((n=b.find(l=>{var p;return(p=o.descricao)==null?void 0:p.startsWith(`[${l.titulo}]`)}))==null?void 0:n.id)||null,direcao:o.direcao||"enviado",texto:o.direcao==="recebido"?o.descricao:((i=o.descricao)==null?void 0:i.replace(/^\[.*?\]\s*/,""))||o.descricao,canal:o.tipo==="whatsapp_recebido"?"recebido":"enviado",ts:o.created_at}})}catch{}const a=c==null?void 0:c.querySelector("#chat-hist");a&&(a.innerHTML=k(),a.scrollTop=a.scrollHeight);const t=c==null?void 0:c.querySelector(".etapas-lista");t&&(t.innerHTML=_())}function Ja(){s._channel=Sa(s.lead.id,a=>{if(s.mensagens.some(o=>o.ts===a.created_at&&o.texto===a.descricao))return;s.mensagens.push({etapa:null,direcao:a.direcao||"recebido",texto:a.descricao,canal:a.tipo==="whatsapp_recebido"?"recebido":"enviado",ts:a.created_at});const e=c==null?void 0:c.querySelector("#chat-hist");e&&(e.innerHTML=k(),e.scrollTop=e.scrollHeight)})}function P(){var a,t;s._channel&&(s._channel.unsubscribe(),s._channel=null),(a=c==null?void 0:c.querySelector("#conversa-modal"))==null||a.remove(),c=null,(t=s.onClose)==null||t.call(s)}function na({lead:a,profile:t,container:e,onClose:o,onLeadUpdated:n}){s.lead=a,s.profile=t,s.onClose=o,s.onLeadUpdated=n,s.etapaAtual=0,s.mensagens=[],s.enviando=!1,s.melhorando=!1,s.textos={};const i=A[a.beneficio]||A.outros;s.docsEscolhidos=[...i],c=e;const l=document.createElement("div");l.innerHTML=Va(),e.appendChild(l.firstElementChild),Wa(),Ya(),Ja()}const Xa={aposentadoria:"Aposentadoria","auxilio-doenca":"Auxílio-doença","bpc-loas":"BPC/LOAS","beneficio-negado":"Ben. Negado",outros:"Outros"},Za={"primeiro-pedido":"1º Pedido","em-analise":"Em análise",indeferido:"Indeferido",cessado:"Cessado"},ae={novo:"Novo","em-contato":"Em contato",qualificado:"Qualificado",convertido:"Convertido",descartado:"Descartado"},r={tab:"leads",leads:[],usuarios:[],loading:!1,loadingUsuarios:!1,erro:"",erroUsuarios:"",filtroClassificacao:"",filtroStatus:"novo",filtroBeneficio:"",modalNovoUsuario:!1,salvandoUsuario:!1,erroNovoUsuario:""};let d=null,g=null;function ia(a){return a?new Date(a).toLocaleString("pt-BR",{day:"2-digit",month:"2-digit",year:"numeric",hour:"2-digit",minute:"2-digit"}):"-"}function ee(a){return String(a||"").replace(/^\+/,"")}function ra(){return r.leads.filter(a=>!(r.filtroClassificacao&&a.classificacao!==r.filtroClassificacao||r.filtroStatus&&a.status_atendimento!==r.filtroStatus||r.filtroBeneficio&&a.beneficio!==r.filtroBeneficio))}function te(a){return`<span class="chip ${{Alta:"chip-alta",Média:"chip-media",Baixa:"chip-baixa"}[a]||"chip-baixa"}">${a}</span>`}function oe(a){return`<span class="badge ${{novo:"badge-novo","em-contato":"badge-em-contato",qualificado:"badge-qualificado",convertido:"badge-convertido",descartado:"badge-descartado"}[a]||"badge-novo"}">${ae[a]||a}</span>`}function se(){return`
        <header class="ad-header">
            <div class="ad-header-left">
                <span class="ad-icon">⚖️</span>
                <div>
                    <h1 class="ad-title">Painel Administrativo</h1>
                    <span class="ad-sub">Olá, ${(g==null?void 0:g.nome)||"Admin"} · Administrador</span>
                </div>
            </div>
        ...
        </header>`}function ca(a){const t=a.length,e=a.filter(i=>i.classificacao==="Alta").length,o=a.filter(i=>i.status_atendimento==="em-contato").length,n=a.filter(i=>i.status_atendimento==="convertido").length;return`
        <div class="ad-stats">
        ...
        <div class="ad-filtros">
            ...
            <button id="btn-refresh" class="btn-outline" style="margin-left:auto">↻ Atualizar</button>
        </div>`}function da(a){return r.loading?'<div class="ad-feedback ad-loading">Carregando leads…</div>':r.erro?`<div class="ad-feedback ad-erro">${r.erro}</div>`:a.length?`
        <div class="table-wrap">
            <table class="leads-table">
                <thead><tr>
            ...
            <tr data-id="${e.id}">
                ...
                <td class="td-data">${ia(e.created_at)}</td>
                <td class="td-acoes">
                    ${n?`
        ...
        </div>`:'<div class="ad-feedback ad-vazio">Nenhum lead encontrado.</div>'}function ie(){const a=ra();return`
        ${ca(r.leads)}
        ${ne()}
        <section class="ad-table-section" id="leads-section">
            ...
            ${da(a)}
        </section>`}function la(){return r.modalNovoUsuario?`
        ...
        </div>`:""}function U(){if(r.loadingUsuarios)return'<div class="ad-feedback ad-loading" style="margin:24px">Carregando usuários…</div>';if(r.erroUsuarios)return`<div class="ad-feedback ad-erro" style="margin:24px">${r.erroUsuarios}</div>`;const t=r.usuarios.map(e=>{const o=e.id===(g==null?void 0:g.id);return`
            <tr>
                <td>
                    <div class="lead-nome">${e.nome||"—"}</div>
        ...
        ${la()}`}function re(){return`
        <div class="ad-layout">
            ${se()}
            <div class="ad-tab-content">
                ${r.tab==="leads"?ie():U()}
            </div>
        </div>`}function S(){const a=d==null?void 0:d.querySelector("#leads-section");if(!a)return;const t=ra();a.querySelector("h2").innerHTML=`Leads <span class="count-badge">${t.length}</span>`;const e=a.querySelector(".table-wrap, .ad-feedback"),o=document.createElement("div");o.innerHTML=da(t);const n=o.firstElementChild;e&&n?e.replaceWith(n):n&&a.appendChild(n),fa()}function ua(){const a=d==null?void 0:d.querySelector(".ad-stats");if(!a)return;const t=document.createElement("div");t.innerHTML=ca(r.leads),a.replaceWith(t.firstElementChild)}function N(){var a;if((a=d.querySelector("#modal-usuario"))==null||a.remove(),r.modalNovoUsuario){const t=document.createElement("div");t.innerHTML=la(),d.querySelector(".ad-layout").appendChild(t.firstElementChild),ce()}}function pa(){d&&(d.innerHTML=re(),de())}function fa(){d.querySelectorAll(".btn-atender").forEach(a=>a.addEventListener("click",()=>pe(a.dataset))),d.querySelectorAll(".status-select:not(.role-select)").forEach(a=>a.addEventListener("change",()=>ue(a)))}function O(){var a;(a=d.querySelector("#btn-novo-usuario"))==null||a.addEventListener("click",()=>{r.modalNovoUsuario=!0,r.erroNovoUsuario="",N()}),d.querySelectorAll(".role-select").forEach(t=>t.addEventListener("change",()=>fe(t))),d.querySelectorAll("[data-ativo]").forEach(t=>t.addEventListener("click",()=>me(t)))}function ce(){var t,e,o;const a=()=>{r.modalNovoUsuario=!1,r.erroNovoUsuario="",N()};(t=d.querySelector("#modal-user-close"))==null||t.addEventListener("click",a),(e=d.querySelector("#modal-user-cancel"))==null||e.addEventListener("click",a),(o=d.querySelector("#form-novo-usuario"))==null||o.addEventListener("submit",ve)}function de(){var a,t,e,o,n;(a=d.querySelector("#btn-logout"))==null||a.addEventListener("click",async()=>{await B(),window.location.reload()}),d.querySelectorAll(".ad-tab").forEach(i=>i.addEventListener("click",()=>le(i.dataset.tab))),r.tab==="leads"&&((t=d.querySelector("#btn-refresh"))==null||t.addEventListener("click",j),(e=d.querySelector("#f-class"))==null||e.addEventListener("change",i=>{r.filtroClassificacao=i.target.value,S()}),(o=d.querySelector("#f-status"))==null||o.addEventListener("change",i=>{r.filtroStatus=i.target.value,S()}),(n=d.querySelector("#f-benef"))==null||n.addEventListener("change",i=>{r.filtroBeneficio=i.target.value,S()}),fa()),r.tab==="usuarios"&&O()}function le(a){r.tab=a,pa(),a==="leads"&&j(),a==="usuarios"&&ma()}async function ue(a){const{id:t,current:e}=a.dataset,o=a.value;if(o!==e)try{await G(t,o);const n=r.leads.find(i=>i.id===t);n&&(n.status_atendimento=o),a.dataset.current=o,ua()}catch(n){alert(`Erro ao atualizar status: ${n.message}`),a.value=e}}async function pe({id:a}){const t=r.leads.find(e=>e.id===a);t&&na({lead:t,profile:g,container:d,onLeadUpdated:e=>{const o=r.leads.findIndex(n=>n.id===e.id);o!==-1&&(r.leads[o]={...r.leads[o],...e}),S()},onClose:()=>S()})}async function fe(a){const{id:t,current:e}=a.dataset,o=a.value;if(o!==e){if(!confirm(`Alterar perfil para "${o}"?`)){a.value=e;return}try{await qa(t,o);const n=r.usuarios.find(i=>i.id===t);n&&(n.role=o),a.dataset.current=o}catch(n){alert(`Erro: ${n.message}`),a.value=e}}}async function me(a){const{id:t,ativo:e}=a.dataset,o=e!=="true";if(confirm(`Deseja ${o?"ativar":"desativar"} este usuário?`))try{await Aa(t,o);const n=r.usuarios.find(l=>l.id===t);n&&(n.ativo=o);const i=d.querySelector(".ad-tab-content");i&&(i.innerHTML=U(),O())}catch(n){alert(`Erro: ${n.message}`)}}async function ve(a){if(a.preventDefault(),r.salvandoUsuario)return;const t=d.querySelector("#nu-nome").value.trim(),e=d.querySelector("#nu-email").value.trim(),o=d.querySelector("#nu-senha").value,n=d.querySelector("#nu-role").value;r.salvandoUsuario=!0,r.erroNovoUsuario="",N();try{await $a({nome:t,email:e,password:o,role:n}),r.modalNovoUsuario=!1,r.salvandoUsuario=!1,await ma()}catch(i){r.salvandoUsuario=!1,r.erroNovoUsuario=i.message,N()}}async function j(){r.loading=!0,r.erro="",S();try{r.leads=await W()}catch(a){r.erro=a.message}finally{r.loading=!1,ua(),S()}}async function ma(){r.loadingUsuarios=!0,r.erroUsuarios="";const a=d==null?void 0:d.querySelector(".ad-tab-content");a&&r.tab==="usuarios"&&(a.innerHTML=U());try{r.usuarios=await wa()}catch(t){r.erroUsuarios=t.message}finally{r.loadingUsuarios=!1,a&&r.tab==="usuarios"&&(a.innerHTML=U(),O())}}function be(a,t){d=a,g=t,pa(),j()}const h=document.querySelector("#admin-app");function T(a=""){h.innerHTML=`
        <div class="al-wrap">
            <div class="al-card">
        ...
        </div>`,h.querySelector("#login-form").addEventListener("submit",async t=>{t.preventDefault();const e=h.querySelector("#login-btn"),o=h.querySelector("#login-email").value.trim(),n=h.querySelector("#login-senha").value;e.disabled=!0,e.textContent="Entrando…";try{const{profile:i}=await ha(o,n);va(i)}catch(i){T(i.message)}})}function va(a){if(!(a!=null&&a.ativo)){B(),T("Usuário inativo. Contate o administrador.");return}a.role==="admin"?be(h,a):Ha(h,a)}async function he(){if(!u){h.innerHTML=`
            <div class="al-wrap">
                <div class="al-card">
                    <div class="al-icon">⚠️</div>
            ...
            </div>`;return}h.innerHTML='<div class="al-wrap"><div class="al-card" style="text-align:center;color:#6b7280">Verificando sessão…</div></div>';try{const a=await ga();a!=null&&a.profile?va(a.profile):T()}catch{T()}}he();
